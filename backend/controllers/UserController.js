...
class UserController{
    ...
    async create(req, res){
        var {name, username, password, id_municipio, email, role, id_prop } = req.body;
        ...
        await User.new(name, username, password, id_municipio, email, role, id_prop);
...
//console.log(user.old_password);
    ...
    async endereco(req, res){
        try {
            var { id_user, cep, logradouro, numero, complemento, bairro, cidade, uf, alias } = req.body;

            await User.endereco(id_user, cep, logradouro, numero, complemento, bairro, cidade, uf, alias);
    ...
    async enderecos(req, res) {
        ...
        var ends = await User.findEnderecos(id);
        res.json(ends);
    }
        ...
        try {
            ...
            if(user != undefined){
                ...
                if(resultado){
                    ...
                    var token = jwt.sign({ id: user.id_usuario, name: user.name, email: user.email, role: user.role }, secret, { expiresIn: 3600});

                    res.status(200);
                    res.json({name: user.name, role: user.role, id: user.id_usuario, token: token});
                   // res.json({user: {name: user.name, role: user.role}, token: token});
                }else{
                    res.status(406);
