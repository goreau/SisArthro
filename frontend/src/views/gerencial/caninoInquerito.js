...
export function gerarPDFRisco(data) {
    ...
    const columns = [
        ...
        { header: "Endereço", dataKey: "endereco", width: 50 },
        { header: "RA", dataKey: "ra", width: 10 },
        { header: "Nome", dataKey: "nome", width: 20 },
        { header: "Situação", dataKey: "situacao", width: 12 },
        { header: "Data", dataKey: "dt_col", width: 10 },
        { header: "Data", dataKey: "dt_dpp", width: 10 },
        { header: "Result", dataKey: "res_dpp", width: 8 },
        { header: "Data", dataKey: "dt_elisa", width: 10 },
        { header: "Result", dataKey: "res_elisa", width: 8 },
        { header: "Final", dataKey: "res_fim", width: 8 },
        { header: "Ciclo", dataKey: "ciclo", width: 8 },
        { header: "Data", dataKey: "dt_coleira", width: 8 },
        { header: "Situação", dataKey: "sit_coleira", width: 10 },
        { header: "Código", dataKey: "cod_desf", width: 8 },
        { header: "Data", dataKey: "dt_desf", width: 10 },
    ];
    ...
    for (const linha of data) {
        if (cont++ == 22) {
            ...
            doc.autoTable({
                head: [
                    [
                        { content: "Imóvel", colSpan: 2, styles: { halign: 'center', } },
                        { content: "Animal", colSpan: 2, styles: { halign: 'center', } },
                        { content: "Coleta", colSpan: 2, styles: { halign: 'center', } },
                        { content: "DPP", colSpan: 2, styles: { halign: 'center', } },
                        { content: "Elisa", colSpan: 2, styles: { halign: 'center', } },
                        { content: "Result", colSpan: 1, styles: { halign: 'center', } },
                        { content: "Coleira", colSpan: 3, styles: { halign: 'center', } },
                        { content: "Desfecho", colSpan: 2, styles: { halign: 'center', } }
                    ],
                ...
                margin: { top: 32, right: 5, bottom: 10, left: 5 },
                styles: {
...
function addHeader(doc, cabeca) {
    ...
    let texto = 'BOLETIM DE COLETA E REGISTRO DE RESULTADO DE EXAME LABORATORIAL E  CONTROLE  EM ATIVIDADE DE INQUÉRITO CANINO';
    ...
    let txtCabeca = `Município: ${cabeca.municipio}      Área: ${cabeca.area}        Quarteirão: ${cabeca.quarteirao}                       Nº Inquérito: ________/________`;
    doc.text(txtCabeca, larguraPagina / 2, 20, null, null, "center");

    doc.setDrawColor(0, 0, 0);
    let posXx = 50; // Posição X inicial
    let posY = 25; // Posição Y inicial
    const rectWidth = 4; // Largura do retângulo
    const rectHeight = 4; // Altura do retângulo
    const textOffset = 2;

    doc.rect(posXx, posY, rectWidth, rectHeight);

    doc.text("Tipo de Inquérito", posXx - textOffset -3, posY + rectHeight / 2 + 1,null, null, "right");
    // Adicionar texto ao lado do retângulo
    doc.text("Amostral", posXx + rectWidth + textOffset, posY + rectHeight / 2 + 1);

    posXx = 80; // Posição X inicial
    doc.rect(posXx, posY, rectWidth, rectHeight);
    doc.text("Censitário", posXx + rectWidth + textOffset, posY + rectHeight / 2 + 1);


    posXx = 180;
    doc.rect(posXx, posY, rectWidth, rectHeight);

    doc.text("Atividade com encoleiramento", posXx - textOffset -3, posY + rectHeight / 2 + 1,null, null, "right");
    // Adicionar texto ao lado do retângulo
    doc.text("Sim", posXx + rectWidth + textOffset, posY + rectHeight / 2 + 1);

    posXx = 195; // Posição X inicial
    doc.rect(posXx, posY, rectWidth, rectHeight);
    doc.text("Não                Nº Cães ATL: _______", posXx + rectWidth + textOffset, posY + rectHeight / 2 + 1);
}
