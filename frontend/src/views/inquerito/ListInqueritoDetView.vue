...
export default {
    ...
    mounted() {
        ...
        this.isLoading = true;
        inqueritoService.getInqueritosDet(this.master)
            .then((response) => {               
                this.quart = response.data[0].id_quarteirao;
                if (response.data[0].codend){
            ...
            },
            {
                ...
                formatter: (cell, formatterParams) => {
                    ...
                    btEdit.addEventListener("click", () => {
                        this.$router.push(`/editInqueritoDet/${row.id_inquerito_det}`);
                    });
                    ...
                    btDel.addEventListener("click", async () => {
                        const ok = await this.$refs.confirmDialog.show({
                            title: 'Excluir',
                            message: 'Deseja mesmo excluir essa inquerito e todas as informações associada a ela?',
                            okButton: 'Confirmar',
                        })
                        if (ok) {
